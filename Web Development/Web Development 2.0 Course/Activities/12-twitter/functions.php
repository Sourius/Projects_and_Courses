		...
		if (mysqli_num_rows($result) == 0) {
			...
			while ($row = mysqli_fetch_assoc($result)) {
				...
				if(isset($_SESSION['id'])){
					$isFollowingQuery = "SELECT * FROM followers WHERE follower = ". mysqli_real_escape_string($link, $_SESSION['id'])." AND isFollowing = ". mysqli_real_escape_string($link, $row['userId'])." LIMIT 1";
