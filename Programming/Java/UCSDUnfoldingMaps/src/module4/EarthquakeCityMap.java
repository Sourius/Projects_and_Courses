...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		y += 20;
		fill(color(200, 160, 0));
		triangle(x,y,x-5,y+10, x+10,y+10);
		fill(0, 0, 0);
		text("City Marker", x+25, y+5);
		
		y += 20;
		fill(255,255,255);
		ellipse(x, y, 10, 10);
		fill(0,0,0);
		text("LandQuake", x+25, y);
		
		y+=10;
		fill(255,255,255);
		rect(x-5,y,10,10);
		fill(0,0,0);
		text("Ocean Quake", 75, y+5);
		...
		*/
		y+=30;
		fill(0,0,0);
		text("Size - Magnitude", x, y);
		
		y+=20;
		fill(255,255,0);
		ellipse(x+5, y+5, 10, 10);
		fill(0,0,0);
		text("Shallow", x+25, y+5);
		
		y+=20;
		fill(0,0,255);
		ellipse(x+5, y+5, 10, 10);
		fill(0,0,0);
		text("Intermediate", x+25, y+5);
	...
	private void printQuakes() 
	{
		...
		Map<String, Integer> countryQuakesMap = new HashMap<>();
		for(Marker m: quakeMarkers) {
			EarthquakeMarker em = (EarthquakeMarker) m;
			String place = "";
			
			if(em.isOnLand()) {
				place = (String)m.getProperty("country");
			}
			else {
				place = "OCEAN QUAKES";
			}
			int counter = 1;
			if(countryQuakesMap.containsKey(place)) {
				counter += countryQuakesMap.get(place);
			}
			countryQuakesMap.put(place,counter);
		}
		
		for(String country: countryQuakesMap.keySet()) {
			System.out.println(country+": "+countryQuakesMap.get(country));
		}
	}
